...
def find_repeated_issues():
    ...
    for _,row in tickets.fillna('').iterrows():
        text=' '.join(str(row.get(c,'')) for c in ['subject','description','resolution']).lower()
        if 'delay' in text or 'late' in text:
            issue='shipment delay'
        elif 'http 500' in text or 'shipment creation' in text:
            issue='shipment creation issue'
        else:
            continue
        groups.setdefault(issue,[]).append({'account_id':row.get('account_id',''),'ticket_id':row.get('ticket_id','')})
    result=[]
    for issue,tickets_for_issue in groups.items():
        customers=set(x['account_id'] for x in tickets_for_issue)
        if len(customers)>=2:
            result.append({'issue':issue,'tickets':tickets_for_issue,'customer_count':len(customers)})
    return result
