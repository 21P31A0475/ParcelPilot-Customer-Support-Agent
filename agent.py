...
import re
from config import OPENAI_MODEL, ALLOWED_ROLE
...
async def Build_Agent():
    ...
    async def First_lookup(State: AgentState):
        ...
        if ticket_ids and 'ticket_lookup' in tool_map:
            ...
            new_messages.append(HumanMessage(content='Preflight ticket evidence:\n' + json.dumps(result, default=str)))
            return {'messages': new_messages}

        if order_ids and 'order_lookup' in tool_map:
            result = await tool_map['order_lookup'].ainvoke({'order_id': order_ids[0], 'account_id': ''})
            new_messages.append(HumanMessage(content='Preflight order evidence:\n' + json.dumps(result, default=str)))
            return {'messages': new_messages}
        ...
        if customer and 'account_lookup' in tool_map:
            ...
            new_messages.append(HumanMessage(content='Preflight account evidence:\n' + json.dumps(result, default=str)))
        ...
        if any(word in low for word in document_words) and 'document_search' in tool_map:
            ...
            print("\n")
            new_messages.append(HumanMessage(content='Preflight document evidence:\n' + json.dumps(result, default=str)))
...
async def main():
    ...
    while True:
        ...
        result = await chatbot.ainvoke(
            {'messages': [HumanMessage(content=query)], 'action': {}, 'approved': False, 'role': ALLOWED_ROLE},
            config=config
        )
